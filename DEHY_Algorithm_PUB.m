...
% Code authors: Lyudmila Khakimova, Yuri Podladchikov, Stefan Schmalholz (November 2023)
...
for icase = 1:2
    ...
    drhos_r     = (drhosCim_r>0)*drhosCim_r - (drhosCim_r<0)*drhosCim_r/4; % Making sure that total density increases with pressure
    ...
    % Initialization of density and porosity versus pressure relations
    ...
    while time < ttime
        ...
        for iter = 1:niter
            ...
            dtiter  = CFL*dx^2/(max(k_etaf)/beta_tot);              % Pseudo-transient step
            ...
            if max(abs(ResPf))<tol && iter>1,break,end
        end
        divVs       = (Pf-Pt)/eta_phi;                              % Divergence of solid velocity
        Vs          = [0 cumsum(divVs*dx)];                         % Solid velocity
